...
def git_clone(url, dir, name, commithash=None):
    ...
    if os.path.exists(dir):
        ...
        try:
            ...
            # If fetch/checkout fails (e.g., different repo or network issue), skip update
            print(f"Warning: Could not update {name} repository. Using existing version.")
    ...
    try:
        ...
        # Don't delete directory if it already exists (e.g., manually cloned)
...
def prepare_environment():
    ...
    # Clone taming-transformers repository
    taming_repo = os.environ.get('TAMING_REPO', 'https://github.com/CompVis/taming-transformers.git')
    try:
        git_clone(taming_repo, repo_dir('taming-transformers'), "Taming Transformers", None)
    except RuntimeError as e:
        print(f"Warning: Could not clone Taming Transformers repository: {e}. Continuing anyway...")

    git_clone(assets_repo, repo_dir('stable-diffusion-webui-assets'), "assets", assets_commit_hash)
    try:
        git_clone(stable_diffusion_repo, repo_dir('stable-diffusion-stability-ai'), "Stable Diffusion", stable_diffusion_commit_hash)
    except RuntimeError as e:
        print(f"Warning: Could not clone Stable Diffusion repository: {e}. Continuing anyway...")
...
def start():
    ...
    # Add taming-transformers to path before importing webui
