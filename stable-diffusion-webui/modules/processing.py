...
class StableDiffusionProcessing:
    ...
    def depth2img_image_conditioning(self, source_image):
        ...
        if AddMiDaS is None:
            raise RuntimeError("AddMiDaS is not available. ldm.data.util module is missing.")
...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    # 한글 프롬프트 자동 번역 (설정이 켜져 있고 한글이 포함된 경우)
    if getattr(opts, 'translate_korean_prompts', True):
        try:
            from modules.korean_prompt_translate import translate_prompts_if_needed, has_hangul
            
            # 한글이 있는지 확인
            prompt_has_hangul = has_hangul(p.prompt) if isinstance(p.prompt, str) else any(has_hangul(str(p)) for p in p.prompt) if isinstance(p.prompt, list) else False
            neg_has_hangul = has_hangul(p.negative_prompt) if isinstance(p.negative_prompt, str) else any(has_hangul(str(p)) for p in p.negative_prompt) if isinstance(p.negative_prompt, list) else False
            
            if prompt_has_hangul or neg_has_hangul:
                print(f"\n[한글 프롬프트 감지] 번역 시작...")
                p.prompt, p.negative_prompt = translate_prompts_if_needed(
                    p.prompt, p.negative_prompt, enabled=True
                )
                print(f"[한글 프롬프트 번역 완료]\n")
        except Exception as e:
            logging.warning(f"한글 프롬프트 번역 중 오류 발생: {e}")
